class TmpMemberInfo < ApplicationRecord
  ...
  with_options if: -> { current_step == "base" } do
    validates :login_id, format: { with: /\A[a-zA-Z0-9]+\z/, message: "は半角英数字のみで入力してください。" }
  ...
  with_options if: -> { current_step == "address" } do
    validates :zip_str3, format: { with: /\A[0-9]{3}\z/, message: "は3桁の数字で入力してください。" }
    validates :zip_str4, format: { with: /\A[0-9]{4}\z/, message: "は4桁の数字で入力してください。" }
